...
ggplot() +
  geom_curve(aes(x=.28,xend=.25,y=0,yend=.125), linewidth=1) +
  geom_segment(aes(x=a,xend=a,y=0,yend=a*.9), linewidth=1, linetype='dotted') +
  ...
  # annotate("text", label = expression(f["casc-min"]), x = a, y = .25,size=6) +
  annotate("text", label = expression(S["thr"]), x = a, y = .25,size=6) +
  annotate("text", label = expression(theta), x = .295, y = .125,size=6) +
