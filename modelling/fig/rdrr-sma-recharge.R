...
ggplot(data.frame(x = 0),aes(x=x)) + # dummy dataframe
  ...
  # Daily
  annotate("text", x = .1e-7, y = 0.3, parse = TRUE,
           label = "1 - e ^ {-K[sat]*Delta*t / L}") +
  annotate("text", x = .3e-8, y = 0.55, parse = TRUE,
           label = "K[sat]*Delta*t / L", color='red') +
  annotate("text", x = .5e-9, y = 0.75, label = "L = 0.1mm") +
  annotate("text", x = .5e-6, y = 0.75, label = "L = 10mm") + 
...
ggsave('modelling/fig/rdrr-sma-recharge.png', width=8, height=4)
